...
from room_data import Coordinate
...
def generate_room(length: float, width: float) -> Dict:
    """
...
def generate_lights(room: Dict, room_type: str) -> tuple[List[Dict], List[Dict]]:
    ...
    if room_type == "1":
        ...
        length_light_count = (
            int((length - 2 * MIN_BORDER_MARGIN_1) / MIN_LIGHT_SPACING_1) + 1
        )
        width_light_count = (
            int((width - 2 * MIN_BORDER_MARGIN_1) / MIN_LIGHT_SPACING_1) + 1
        )
        ...
        for i in range(0, length_light_count):
            for j in range(0, width_light_count):
                ...
                if i < 2 or j < 2:
                    light_pattern.append({"x": x, "y": y})
                light_layout.append({"x": x, "y": y})
    elif room_type == "2":
        ...
        length_light_count = (
            int((length - 2 * MIN_BORDER_MARGIN_2) / MIN_LIGHT_SPACING_2) + 1
        )
        width_light_count = (
            int((width - 2 * MIN_BORDER_MARGIN_2) / MIN_LIGHT_SPACING_2) + 1
        )
        length_light_interval = (length - 2 * MIN_BORDER_MARGIN_1) / length_light_count
        width_light_interval = (width - 2 * MIN_BORDER_MARGIN_1) / width_light_count
        ...
        for i in range(0, length_light_count):
            for j in range(0, width_light_count):
                ...
                if i == 0 or i == length_light_count - 1:
                    if j < 2:
                        light_pattern.append({"x": x, "y": y})
                    light_layout.append({"x": x, "y": y})
                elif j == 0 or j == length_light_count - 1:
                    if i < 2:
                        light_pattern.append({"x": x, "y": y})
                    light_layout.append({"x": x, "y": y})
    else:
...
def save_room_to_file(room: Dict, filename: str) -> None:
    """
